...
A: The performance of pipeline benchmarking strongly depends on the models. For `yolo11n` object detection specifically, it is recommended to use the model precision FP32 when it is running on a `GPU` device. If supported, you can then change the model precision by updating the pipeline script to point to the precision of your choice. For example, you can change the model from `FP16` to `FP32`, assuming the precision `FP32` of the target model is available:
...
A: Some systems may run into memory issues when building the `dlstreamer-realsense` image. In the `Dockerfile.dlstreamer` file, change the make command to not use the `-j` threading option.
...
A: Depending on your system's performance it is possible that the RTSP simulator may take additional time to initialize and start streaming. To avoid issues you can add a waiting period before the pipeline starts. For example, you can add a 5 second sleep timer to /src/entrypoint.sh.
...
## Q: How to configure the centralized proxy?
